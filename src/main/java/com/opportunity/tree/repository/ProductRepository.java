...
public interface ProductRepository extends ReactiveCrudRepository<Product, Long>, ProductRepositoryInternal {
    ...
    @Query("SELECT * FROM product entity WHERE entity.team_id = :id")
    Flux<Product> findByTeam(Long id);

    @Query("SELECT * FROM product entity WHERE entity.team_id IS NULL")
    Flux<Product> findAllWhereTeamIsNull();
