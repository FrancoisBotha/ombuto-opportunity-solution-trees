# Getting Started with ombuto-opportunity-solution-tree
...
_what you're building_. Only after that does code get written.
...
mode is where you define _what_ the system should do; Build mode is where
...
```bash
gh repo create ombuto-opportunity-solution-tree --private --source=. --remote=origin --push
```
...
2. Set the repository name to `ombuto-opportunity-solution-tree` (or whatever you like).
...
`<repo-url-from-github>` looks like `https://github.com/<you>/ombuto-opportunity-solution-tree.git`
for HTTPS, or `git@github.com:<you>/ombuto-opportunity-solution-tree.git` for SSH.
