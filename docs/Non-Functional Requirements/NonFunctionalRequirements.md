...
| NFR-022 | Transcripts | Privacy: transcript bodies may contain customer personal data — they are only returned to members of the owning team, never written to application logs, and never included in WebSocket events. | NEW | epic_12_MEETING_TRANSCRIPTS |
| NFR-023 | Transcripts | Security: uploads are validated server-side by extension and size limit before parsing, parsed as text only, and transcript bodies are rendered as plain text and never as HTML; all reads and writes are scoped through `TeamAccessService`. | NEW | epic_12_MEETING_TRANSCRIPTS |
| NFR-024 | Transcripts | Performance: transcript bodies are never included in the tree payload or in any list response — those carry counts and metadata only — and the body is fetched only when the viewer dialog opens, so NFR-004's 2-second tree render budget holds unchanged. | NEW | epic_12_MEETING_TRANSCRIPTS |
