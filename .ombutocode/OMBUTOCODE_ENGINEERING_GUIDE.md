...
## 4b. Data Model Changes — JDL and the JHipster Generator (MANDATORY)

This application is generated by JHipster. **`ombuto.jdl` at the repository root is the
single source of truth for the data model.** Every data model change — adding, renaming or
removing an entity, field, enum, relationship, validation rule, or an entity option such as
`dto`, `service`, `paginate` or `filter` — MUST be made by editing `ombuto.jdl` and then
running the JHipster generator. No exceptions for "small" changes.

### Required procedure

1. Edit `ombuto.jdl`. Nothing else yet.
2. Make sure the working tree is committed or stashed, so the generator's output can be
   reviewed as a clean diff.
3. Run the generator from the repository root:

   ```bash
   npx jhipster jdl ombuto.jdl
   ```

   Use the project-local generator (`generator-jhipster` in `package.json`), never a globally
   installed `jhipster` of a different version.
4. Review the full `git diff`. The generator owns, and will rewrite, all of:
   `.jhipster/<Entity>.json`, the JPA entity, repository, service, query service, criteria,
   DTO, mapper and REST resource, the Liquibase changelog and fake-data CSV, the Vue entity
   screens, model and service under `src/main/webapp/app/entities/` and `shared/model/`, and
   the generated tests (`*Test`, `*ResourceIT`, `*TestSamples`, `*Asserts`, `*.spec.ts`).
5. When the generator reports a conflict with a hand-customised file, resolve it file by file
   and keep the customisation. Do NOT pass `--force` to overwrite blindly.
6. Run the tests for the affected entities as described in
   `docs/Test Strategy/test-strategy.md`. The generated tests are part of the change and MUST
   pass — do not delete, skip or weaken them to get a ticket through.
7. Commit `ombuto.jdl`, `.jhipster/*.json` and the generated files together, in one commit.

### Agents MUST NOT

- Hand-edit a JPA entity, DTO, criteria class, Liquibase entity changelog or
  `.jhipster/*.json` to change the data model. If it is not in `ombuto.jdl`, it does not exist.
- Hand-write the CRUD stack (repository / service / resource / Vue screens / tests) for a new
  entity instead of generating it.
- Let `ombuto.jdl`, `.jhipster/*.json` and the code drift apart. If they already disagree,
  STOP and ask — do not pick a winner.
- Use SQL reserved words as entity or field names (`value`, `order`, `key`, `user`, `group`,
  `rank`, …). PostgreSQL tolerates some that H2 rejects, which breaks the whole H2 test run.

### Liquibase changelogs

- **Before the first production deployment** (the current state — incremental changelogs are
  not enabled in `.yo-rc.json`), the generator rewrites the original
  `*_added_entity_<Entity>.xml` in place. Existing dev databases then fail Liquibase checksum
  validation and must be wiped: delete `target/h2db/db` (H2) or run `npm run docker:db:down`
  (PostgreSQL). Say so in the ticket notes.
- **Once any environment holds data that must survive**, a deployed changelog MUST NOT be
  edited again. Switch to `npx jhipster jdl ombuto.jdl --incremental-changelog` so each change
  produces a new migration. Making that switch is an architecture decision — ASK first.

### Custom code lives beside generated code, not inside it

Behaviour the generator cannot express (custom queries, business rules, bespoke endpoints, the
tree UI) belongs in separate classes, components and test files, so the next regeneration
cannot clobber it. When a generated file genuinely has to be customised, keep the edit minimal
and list the file in the ticket notes so the next regeneration knows to preserve it.

### Tickets

A data model ticket's acceptance criteria MUST include: `ombuto.jdl` updated, generator run,
generated tests passing. Data model tickets all touch `ombuto.jdl` and the Liquibase
`master.xml`, so treat those as scaffolding files (§4a) and chain such tickets with
`depends_on` — never run two in parallel.
...
- Change the data model by hand — every data model change goes through `ombuto.jdl` and the
  JHipster generator (see §4b)
